 ...
 * that are compatible with the MQLib library, supporting both WASM-based SQLite
 * and native FFI-based SQLite3 implementations.
 */
 ...
 * @param preferNativeOrOptions Whether to prefer the native FFI implementation if available, or configuration options
...
export async function createSqliteConnection(
  ...
  // Handle backwards compatibility case - dbPath and preferNative
  const dbPath = dbPathOrLibrary || ":memory:";
  const preferNative = typeof preferNativeOrOptions === 'boolean' 
    ? preferNativeOrOptions 
    : true;

  // Try to create a connection with the preferred library
  let connection: NormalizedConnection;
  
  if (preferNative) {
    try {
      connection = await createNativeConnection(dbPath);
    } catch (error) {
      console.warn("Native SQLite3 library not available, falling back to WASM implementation:", error);
      connection = await createWasmConnection(dbPath);
    }
  } else {
    try {
      connection = await createWasmConnection(dbPath);
    } catch (error) {
      console.warn("WASM SQLite library not available, trying native implementation:", error);
      connection = await createNativeConnection(dbPath);
    }
  }

  // Return a standardized connection interface
...
/**
 * Creates a connection using the native FFI-based SQLite3 library.
 */
async function createNativeConnection(dbPath: string): Promise<NormalizedConnection> {
  try {
    // Import the SQLite module dynamically
    // Note: Requires --allow-env, --allow-ffi, and --unstable-ffi flags when running
...
/**
 * Creates a connection using the WASM-based SQLite library.
 */
